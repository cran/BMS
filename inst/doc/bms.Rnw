...
\begin{document}
\maketitle
\abstract{This manual is a brief introduction to applied Bayesian Model Averaging with the R package \verb+BMS+. The manual is structured as a hands-on tutorial for readers with few experience in BMA. Readers with more BMA background are advised to consult the table of contents in order to lookup formal representations of the concepts used in BMS.\\
For more information on the BMS package, please refer to \\
\texttt{http://bms.zeugner.eu}.}
\tableofcontents
\clearpage
...
This section reiterates some basic concepts of BMA, and introduces some notation for readers with limited knowledge of BMA. Readers with more experience in BMA should skip this chapter and directly go to section \ref{sec:example}. For a more thorough introduction to BMA, consult \citet{Hoetingetal99}.
...
A problem arises when there are many potential explanatory variables $X$: Which variables $X_\gamma \in \{X\}$ should be then included in the model? And how important are they? The direct approach to do inference on a single linear model that includes all variables is inefficient or even infeasible with a limited number of observations. 

BMA tackles the problem by estimating models for all possible combinations of $X$; its results are a weighted average over all of them. If $X$ contains $K$ potential variables, this means estimating $2^K$ variable combinations and thus $2^K$ models. The model weights for this averaging stem from posterior model probabilities that arise from Bayes' theorem:
\begin{equation}
 p(M_\gamma | y, X) \; = \; \frac{p(y |M_\gamma, y,X) p(M_\gamma)}{p(y|X)} \; \propto p(y |M_\gamma, y, X) p(M_\gamma)	
	\label{eq:bf}
\end{equation}

Here, $p(y|X)$ denotes the \emph{integrated} likelihood which is constant over all models and is thus simply a multiplicative term. Therefore, the posterior model probability (PMP) is proportional (embodied by the sign $\propto$) to the marginal likelihood of the model $p(y |M_\gamma, y, X) $ (the probability of the data given the model $M_\gamma$) times a prior model probability $p(M)$ -- that is, how probable the researcher thinks model $M_\gamma$ before looking at the data. By renormalization of the this product one can infer the PMPs and thus the model weighted posterior distribution for any statistic $\theta$ (e.g. the coefficients $\beta$):
...
The specific expressions for marginal likelihoods $p(M_\gamma|y,X)$ and posterior distributions $p(\theta | M_\gamma, y, X)$ depend on the estimation framework chosen. The literature standard is to use a Bayesian normal-conjugate linear model with a specific prior structure called 'Zellner's g prior' as will be outlined below:\\
...
$$ Cov(\beta_\gamma|y,X,g,M_\gamma) = \frac{(y-\bar{y})'(y-\bar{y})}{N-2} \frac{g}{1+g} \left(1-\frac{g}{1+g} R_\gamma^2\right) (X_\gamma'X_\gamma)^{-1} $$
I.e. the posterior covariance is similar to that of the OLS estimator, times a factor that includes $g$ and $R^2_\gamma$, the OLS R-squared for model $\gamma$.

Finally, the marginal likelihood $p(y|M_\gamma,X,g)$ is related to the R-squared and includes a size penalty factor that adjusts for model size $k_\gamma$:
$$ p(y|M_\gamma,X,g) \propto (y-\bar{y})'(y-\bar{y})^{-\frac{N-1}{2} } (1+g)^{-\frac{k_\gamma}{2}} \left( 1- \frac{g}{1+g} \right)^{-\frac{N-1}{2}} $$
The crucial choice to take here is the form of the hyperparameter $g$. The standard framework is to set $g$ to a fixed value that is common to all models. A popular 'default' specification is the 'unit information prior' (UIP), which sets $g=N$ and thus attributes about the same information to the prior as is contained in one observation.

The Bayesian normal-conjugate model with Zellner's $g$ prior is the workhorse model for BMA, but may also be used as a single model (i.e. without taking model uncertainty into account). Section \ref{ssec:zlm} in the appendix demonstrates the use of the function \verb+zlm+ to estimate such a single model. 
...
Invoking the command \verb+summary(att)+ yielded the important posterior statistic \verb+Mean no. regressors+, the posterior expected value of model size which in our case was $2.11$. Note that the posterior expected model size is equal to the sum of PIPs -- verify via
<<>>=
sum(coef(att)[,1])
@
...
We see that while the model prior implies a symmetric distribution around $K/2=3$, updating with the data points to somewhat smaller models.
...
\subsection{Custom Model Priors}
...
@
But the results (obtained with \verb+coef(att_pip)+) show that \verb+complaints+ still retains its PIP of near 100\%. Instead, posterior model size decreases (as evidenced in a call to \verb+plotModelsize(att_pip)+), and all other variables obtain a far smaller PIP. 
...
With the beta-binomial specification and prior model size $\bar{m}=K/2$, the model prior is completely flat, while the posterior model size turns out to be $1.73$. In terms of coefficient and posterior model size distribution, the results are very similar to those of \verb+att_fixed+, even though the latter approach involved a tighter model prior. Concluding, a decrease of prior importance by the use of the beta-binomial framework supports the results found in \verb+att_fixed+.

Compare the PIPs from the four approaches presented so far with the following command:
<<echo=FALSE>>=

<<fig=TRUE>>=
plotComp(att, att_fixed, att_pip, att_random, cex=2)
@

Here as well, \verb+att_fixed+ (Model 2) and \verb+att_random+ (Model 4) display similar results with PIPs plainly smaller than those of \verb+att+ (Model 1). 
...
pmp.bma(fls1)[1:5,]
@
The numbers in the left-hand column represent analytical PMPs while the right-hand side displays MCMC-based PMPs. Both decline in roughly the same fashion, however the values for analytical PMPs are considerably larger than the MCMC-based ones. This comes from the fact that iteration-based PMPs compare to the entirety of PMPs encountered, while the analytical values were only normalized by the sum of available analytical PMPs (in this case 2,000) -- cf. equation (\ref{eq:bf}).\footnote{In the call to \texttt{topmodels.bma} on page \pageref{topmodcall}, the PMPs under 'MCMC' and analytical ('exact') concepts were equal since 1) enumeration bases both 'top' model calculation and aggregate on-the-fly results on analytical PMPs and 2) because all possible models were retained in the object \texttt{att}.} In order to see the importance of all 'top models' with respect to the full model space, we can thus sum up their MCMC-based PMPs as follows:
<<>>=
colSums(pmp.bma(fls1))
@
The second scalar shows that in total, the top 2,000 models account for ca. \Sexpr{round(colSums(pmp.bma(fls1))[[2]],2)*100}\% of posterior model mass.\footnote{Note that this share was already provided in column \texttt{\% Topmodels} resulting from the \texttt{summary} command on page \pageref{sumfls1}.} They should thus provide a rough approximation of posterior results that might or might not be better than the MCMC-based results. For this purpose, compare the best 5 covariates in terms of PIP by analytical and MCMC methods: \verb+coef(fls1)+ will display the results based on MCMC counts.
<<>>=
coef(fls1)[1:5,]
@
...
<<fig=TRUE>>=
density(fls_hyper,reg="Muslim",addons="Eebl")
@
...
Nonetheless, even though \verb+fls_hyper+ and \verb+fls_combi+ applied very different $g$ and model priors, the results for the \verb+Muslim+ covariate are broadly similar: It is unanimously positive, with a conditional expected value somewhat above $0.01$.
...
\appendix
\section{Appendix}
\subsection{Available Model Priors -- Synopsis}
The following provides an overview over the model priors available in \verb+bms+. Default is \verb+mprior="random"+. For implementation details and examples, consult \verb+help(bms)+.
...
The following provides an overview over the $g$ priors available in \verb+bms+. Default is \verb+g="UIP"+. For implementation details and examples, consult \verb+help(bms)+.
...
\begin{itemize}
	...
	\item \emph{Sub-options}: Unit information prior \verb+g="UIP"+ sets $g=N$; \verb+g="BRIC"+ sets $g=\max(N,K)$, a combination of BIC and RIC. Note these two options guarantee asymptotic consistency.
\end{itemize}
...
data(attitude)
@
The full model is obtained by applying the function \verb+zlm+ on the data set and storing the estimation into \verb+att_full+. Zellner's $g$ prior is estimated by the argument \verb+g+ just in the same way as in section \ref{sec:gprior}. 
<<>>=
...
summary(att_full)
@
The results are very similar to those resulting from OLS (which can be obtained via \verb+summary(lm(attitude))+). The less conservative, i.e. the larger $g$ becomes, the closer results get to OLS. But remember that the best model from the BMA application was not the full model but the model containing only the variable \verb+complaints+. The function \verb+as.zlm+ extracts this single model for further analysis, with the argument \verb+model+ specifying the rank-order of the model to be extracted. The following command reads the best model from the BMA results in section \ref{sec:example} into the variable \verb+att_best+.
<<>>=
att_best = as.zlm(att,model=1)
summary(att_best)
@
The resulting log-marginal likelihood shows why this model performed better than the full model
\end{document}